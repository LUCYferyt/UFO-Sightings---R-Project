...
Model fitting
```{r}
multi_fit <- fit(wf_multi, data = train_data)
```


Predictions based on the fitted model, done on the test data.
```{r}
...
Trained model metrics. These are generally pretty poor results
```{r}
...
Trained model metrics. Generally pretty poor results here as well
```{r}
metrics <- metric_set(accuracy, precision, recall, f_meas)
...
Confusion matrix - it clearly shows that the model mainly wants to predict *circle* and *light*. This time however there is definitely an improvement.
```{r}
...
Based on our results we came to the conclusion that the dataset has poor data for any model training. Most of all, the data is not balanced well enough. We first tried to make a model that would try to predict if a sighting was in USA or any other country other than it. That did not work well, as the amount of sightings in USA alone is so high, that it didn't work at all. Then we tried to predict other parameters and this way we finally landed at predicting shapes of the UFOs. Even though the results are still not great, these are the best ones we have gotten out of all of them. That is because even the *shape* data is not really balanced, as most of the sightings reported it the UFO as "light". That is what we believe was the primary reason for these poor results.